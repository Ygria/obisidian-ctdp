                    ...
                    {tasks.map((task) => (
                        ...
                        <div key={task.originalIndex} className="relative p-4 bg-white rounded-lg shadow-sm border">
                            <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <p class="mb-3 font-normal text-gray-500 dark:text-gray-400">{task.rules}</p>
                            ...
                            <div className="flex gap-2 mt-12">
                                <div onClick={() => onTaskStart(task.originalIndex, 'schedule')} className=" relative inline-flex items-center justify-center p-0.5  me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-cyan-300 to-lime-300 group-hover:from-teal-300 group-hover:to-lime-300 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-lime-800">
                                    <span className=" flex items-center relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-transparent group-hover:dark:bg-transparent">
                                        <Timer
                                            className="w-4 h-4 mr-2 text-lime-400" />预约
                                    </span>
                                </div>
                                <div onClick={() => onTaskStart(task.originalIndex, 'immediate')} className="flex items-center text-white bg-gradient-to-r from-cyan-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 "><Play className="w-4 h-4 mr-2 text-white" />
                                    立刻开始</div >
